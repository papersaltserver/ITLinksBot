...
{
    class NodeWeeklyParser : IParser
    {
        private readonly IContentGetter<string> htlmContentGetter;
        private readonly IContentNormalizer contentNormalizer;
        private readonly ITextSanitizer textSanitizer;
        public string CurrentProvider => "Node Weekly";
        readonly Uri baseUri = new("https://nodeweekly.com/");

        public NodeWeeklyParser(IContentGetter<string> cg, IContentNormalizer cn, ITextSanitizer ts)
        {
        ...
        public List<Link> GetDigestLinks(Digest digest)
        {
            ...
            for (int i = 0; i < linksInDigest.Count; i++)
            {
                ...
                string category = link.SelectSingleNode("./preceding-sibling::p[1]")?.InnerText?.Trim();
